...
export async function POST(request: Request) {
  ...
  // Puzzle hints are generated by the model from the student's in-progress
  // attempt, so they need the same 5.1.2(i) consent the learn path checks.
  // Refused BEFORE the tier is logged, so declining never burns a hint tier.
  if (!(await hasAiConsent(user.id))) {
    return NextResponse.json(
      { error: AI_HELP_OFF, code: "ai_consent_required" },
      { status: 403 },
    );
  }
